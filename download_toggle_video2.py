...
class Downloader(threading.Thread):
	...
	def download_file(self, record):
		...
		name = record[0]
		url = record[1]
		folder = record[2].replace("_", " ")
		filename = record[3]
		fileurl = "{}/{}".format(folder, filename)
		...
		if (url.endswith("m3u8")):
			...
			ffmpeg_download_cmd = 'ffmpeg -hide_banner -loglevel info -i ' + url + " -c copy -bsf:a aac_adtstoasc \"" + fileurl + ".mp4\""
		...
		if (url.endswith("mp4") or url.endswith("wvm") or url.endswith("srt")):
			...
			# f = open(file_name, 'wb')
			filetype = subtitleLanguage = re.search(r"(mp4|wvm|srt)",file_name).group(0)
			f = open("{}.{}".format(fileurl, filetype), 'wb')
			meta = u.info()
...
def process_video_url(t_video_url):
	...
	logger.debug("Obtaining show title ...")
	showtitle = re.sub(r'_+-_(\w|\d)+', "", medianame)
	showtitle = showtitle.strip("_")
	try:
		logger.info("Obtained show title = %s" % (showtitle.decode('unicode-escape')))
	except UnicodeEncodeError:
		showtitle = mediaID
		logger.info("Unicode title encountered. New show title = %s" % (showtitle))

	logger.debug("Obtaining show episode ..." + medianame)
	try:
		showEp = re.search(r"(Episode|episode)_(\w|\d)+",medianame).group(0)
	except:
		showEp = False

	if not showEp == False:
		showEp = showEp.split("_")
		showEp = "{}_E{:02}".format(showtitle, int(showEp[1]))
	logger.info("Obtained show episode = %s" % (showEp))

	showfilename = showEp.replace("_", " ")
	...
	for fileInfo in download_url_resp_json.get('Files', []):
		...
		for ext in ["m3u8", "wvm", "mp4"]:
			if urlRecord.startswith('http') and urlRecord.endswith(ext):
				...
				if fileformat:
					temp_urlList.append((medianame+"_"+fileformat[0],urlRecord, showtitle, showfilename))
	...
	if (CHECK_AND_DOWNLOAD_SUBTITLES):
		...
		for sfile in subtitle_link_resp_json.get('subtitleFiles', []):
			...
			queued_urls.append(("Subtitles for "+mediaID,sfile.get('subtitleFileUrl'), showtitle,("{}.{}".format(showfilename, subtitlelanguageShort(sfile.get('subtitleFileLanguage'))))))
			# queued_urls.append(("Subtitles for "+mediaID,sfile.get('subtitleFileUrl')))

	return queued_urls

def subtitlelanguageShort(x):
    return {
        'English': "en_US",
		'Chinese': "zh_CN",
    }.get(x, "en")
